function DATA = Init(DATA, name, TOPTAG, layout)
...
if ~isfield(DATA,'nevdir')
    ...
    DATA.hostconfig = [DATA.gui.prefsdir '/' gethostname '.config'];
    if exist(DATA.hostconfig)
        DATA = ReadConfig(DATA,DATA.hostconfig);
    end
