...
% make sure name is is correct windows/unix form
    ...
    aText.times = [];
    aText.codes = [];
    [aText, Ch30] = AddText(regexprep(name,'\.[0-9]*.mat','Add.txt'), aText, Ch30);
    [aText, Ch30] = AddText(strrep(name,'.mat','Add.txt'), aText, Ch30);
...
if length(id)
    nerr = nerr+1;
    errs{nerr} = sprintf('Bad ISIs (%d), starting at %.0f)\n',length(id),bstimes(id(1)));
    if state.showerrs
        msgbox(errs{nerr},'APlaySpkFile Error!!','modal');
    end
    Result.badisi = id;
...
%combinations
    ...
    if mean(nframes > 2) && state.alltrials == 0
       id = find(nframes > 1);
       Trials = Trials(id);
       bid = find(nframes  ==1);
       if length(bid) > 2
