...
NEV.Data.Tracking = struct('TimeStamp', [], 'TimeStampSec', [], 'MarkerCount', [], 'X', [], 'Y', []);
...
NEV.MetaTags.openNEVver = '4.3.3.0';
...
if exist(matPath, 'file') == 2 && strcmpi(Flags.NoMAT, 'yesmat')
    ...
    load(matPath);
    if WaveData > 0 && isempty(NEV.Data.Spikes.Waveform) && strcmpi(Flags.ReadData, 'read')
...
if strcmpi(Flags.ReadData, 'read')
    ...
    if ~isempty(allExtraDataPacketIndices) % if there is any extra packets
        ...
        if ~isempty(trackingPacketIDIndices)
            ...
            NEV.Data.Tracking.TimeStampSec  = double(NEV.Data.Tracking.TimeStamp)/30000;
            % This portion is commented out because it does not contain any
            % information as of yet.
            NEV.Data.Tracking.ParentID      = tRawData(7:8, trackingPacketIDIndices);
            NEV.Data.Tracking.ParentID      = typecast(NEV.Data.Tracking.ParentID(:), 'uint16').';
            NEV.Data.Tracking.NodeID        = tRawData(9:10, trackingPacketIDIndices);
            NEV.Data.Tracking.NodeID        = typecast(NEV.Data.Tracking.NodeID(:), 'uint16').';
            NEV.Data.Tracking.NodeCount     = tRawData(11:12, trackingPacketIDIndices);
            NEV.Data.Tracking.NodeCount     = typecast(NEV.Data.Tracking.NodeCount(:), 'uint16').';
            NEV.Data.Tracking.MarkerCount    = tRawData(13:14, trackingPacketIDIndices);
            NEV.Data.Tracking.MarkerCount    = typecast(NEV.Data.Tracking.MarkerCount(:), 'uint16').';
            pointCount = max(NEV.Data.Tracking.MarkerCount);
            % We read maximum number of points, valid points is what
            % NEV.Data.Tracking.PointCount shows
            % TODO: right now we only read 2D, find the objectID by iterating the parentsm and
            % figure out if we need 3D coordinates based on NEV.ObjTrackInfo(objectID).TrackableID
            rigidbody = tRawData(15:(15 + 2 * 2 * pointCount - 1), trackingPacketIDIndices);
            rigidbody = typecast(rigidbody(:), 'uint16').';
            NEV.Data.Tracking.X       = rigidbody(1:2:end);
            NEV.Data.Tracking.Y       = rigidbody(2:2:end);
