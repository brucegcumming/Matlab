...
while j <= length(varargin)
    if isstruct(varargin{j}) && isfield(varargin{j},'comment')
        Comments = varargin{j};
    elseif strncmpi(varargin{j},'callback',6)
        j=j+1;
        callback = varargin{j};
...
setappdata(gcf,'Comments',Comments);
...
Comments = getappdata(gcf,'Comments');
if isfield(Comments,'ex')
    id = find([Comments.ex] == ex & [Comments.p] == p);
    for j = 1:length(id)
        text(p, ex+(j-1).*0.5, Comments(id(j)).comment,'color','w');
    end
end
