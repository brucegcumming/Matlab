...
if isnew
    set(F,'menubar','none');
    rmmenu = uimenu(F,'label','Remove','tag','RemoveMenu');
    sm = uimenu(F,'label','Add','tag','AddMenu');
    uimenu(sm,'label','Last Operation',...
        'callback',@AddCallback,...
        'tag','AddCallbackButton');
    if ~isempty(dockcallback)
        uimenu(sm,'label','Dock These Buttons',...
            'callback',dockcallback,...
            'tag','DockButtons');
    end
    if size(X,2) > 2
        a = CellToStruct(X(:,3));
    ...
    for j = 1:size(X,1)
        if X{j,3}.on
            ...
            if ~isempty(it)
                ...
                uimenu(rmmenu,'label',X{j,2},'tag',X{j,1},'callback',{@RemoveButton, X{j,1}});                        
            end
...
if ~isempty(tag) %adding a new button
