    ...
    elseif strncmpi(varargin{j},'Load',4)
        ...
        elseif isdir(DATA.name)
          if strncmpi(varargin{j},'LoadSpikes',8)
            ...
            DATA = LoadAll(DATA,[],'force', warnmode,'expts',expts);
          end
            set(DATA.lstui,'string',DATA.strings);
        else
            fprintf('%s is not a directory\n',DATA.name);
...
%       PlotAllCell(DATA, type); %surely don't need this
    ...
    elseif strncmp(fcn,'allcluster',8)
        if sum(strcmp(type,{'markmanual' 'markauto' 'markcopy' 'markexclusions' 'markgood'}))
...
%clnum 1 = cluster 1, i.e. C.clst == 2
        ...
        for j = 1:length(Expt.Trials)
            ...
            Expt.Trials(j).Spikes = [t(id)-T.Start(1)]';
        end
        E = Expt;
...
function MarkExpts(DATA,type)
    ...
    for j = 1:length(id)
        DrawBox(id(j),[1.3 23.7],3,'color','g','linewidth',1);
    end
...
function SavePlotClustersConfig(DATA,file, varargin);
    savefields = {'showspkxy'  'plotallxy' 'plotspks' 'plothist'...
        'plotcells' 'plotxyseq' 'plot' 'plottrighist', 'comparerates' 'show'};
...
function PlotClusterHistogram(DATA, C, refit, varargin)
    j = 1;
    cl = 1;
    while j <= length(varargin)
...
function h = PlotMeanSpike(C, p, cluster, varargin)
    ...
    elseif plots(1) == 3 %mean lines
        ...
        for j = 1:length(chspk)
            ...
            for j = 1:length(chspk)
                plot(C.MeanSpike.ms(chspk(j),:)+voff(j),'color',colors{j},'linewidth',2);
                hold on;
            end
        end
    end
...
function [f, isnew] = SetFigure(DATA, tag, varargin)
    ...
    if isnew
        ...
        elseif strfind(tag,'CellList');
            ...
            uimenu(hm,'Label','All Spks when change cell','Callback',{@PlotMenu, 'cells' 'watchallspks'});
            uimenu(hm,'Label','All Mean when change cell','Callback',{@PlotMenu, 'cells' 'watchallmean'});
            uimenu(hm,'Label','All XY when change cell','Callback',{@PlotMenu, 'cells' 'watchallxy'});
        ...
        if strmatch(type,{'allspks' 'allcellspks' 'selectspks' 'exptspks' 'allprobespks'})
            ...
            for k = nid;
               ...
               if k < length(h) & h(k+1) > 0 & ishandle(h(k+1))
                            ...
                            color = 1-color;
                            if k > 1
                                offset = std(PC.next{k-1}.MeanSpike.ms(p,:))./4;
                                plot(PC.next{k-1}.MeanSpike.ms(p,:)-offset,'--','color',color,'linewidth',2);
                            else
                                offset = std(PC.MeanSpike.ms(p,:))./4;
                                plot(PC.MeanSpike.ms(p,:)-offset,'--','color',color,'linewidth',2);
                            end
                        end
            end
            end
...
function DATA = InitInterface(DATA)
    ...
    nr = 12;
    nc = 4;
    bp = [0.7+hspace listh+0.1./nr 1./(nc)-hspace 1./nr];
...
function [DATA, Expts]  = LoadExpts(DATA)
    
    
    if exist(DATA.exptname)
        fprintf('Loading %s\n',DATA.exptname);
        [Trials, Expts] = APlaySpkFile(DATA.exptname,'noerrs','nospikes');
        for j = 1:length(Expts)
