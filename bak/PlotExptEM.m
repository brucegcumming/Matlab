...
while j <= nargin-1
    ...
    elseif strncmpi(str,'sumsac',6)
        varargon = {varargon{:} varargin{j}};
    elseif sum(strncmpi(str,{'cmpsac' 'cmppos'},6))
        varargon = {varargon{:} varargin{j}};
...
if ismember(plotmode,[0 7]) %plot individual saccades
    ...
    data.sacs{1} = psacs;
    data.sacs{2} = nsacs;
elseif ismember(plotmode,[2 4 5 6 6 7]) %plot sum of saccades for each trial
    hold off;
    for j = 1:length(Trials)
    ...
    for j = 1:length(pidx)
        ...
        if isempty(S)
            fprintf('Empty Saddacess in Trial %d\n',pidx(j));
        else
