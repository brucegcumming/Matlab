...
if nargin 
    if ischar(varargin{1})
        ...
        elseif  strncmpi(varargin{1},'missedpoint',8)
            ...
            PlotMap(DATA.top,'store',DATA);
        elseif  strncmpi(varargin{1},'gridpoint',4)
            id = varargin{2};
...
if init & isempty(toplevel)
 ...
 %New row
    ...
    bp(1) = bp(1)+bp(3)+HSPACE;
    uicontrol(gcf,'style','pop','string','Dufus|Rufus|Icarus|Lemieux|Dae|Test', 'Tag','RFMonkeyName','Callback',['PlotMap(' top ',''setmonkey'');'], 'position',bp,'value',monkey);
...
elseif monkey == DAE
  ...
  nextfile = '/bgc/bgc/anal/dae/penlist';
  monkeyname = 'dae';
