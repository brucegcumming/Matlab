...
elseif strncmpi(name,'autocut',6)
    ...
    for j = 1:length(eid)
        CombineAutoCut(DATA,DATA.expid(eid(j)),eid(j));
    end
    ...
    if sum(DATA.plot.useprobe)
        ...
        for j = 1:length(eid)
            for k = pid
                ...
                subplot(nr,nc,k);
                DATA = CombineAutoCut(DATA,DATA.expid(eid(j)),eid(j),'cluster',args{:});
            end
        end
        DATA.probe = oldprobe;
    else
...
function P =  NextPlot(P, AllCellRes, step)
    ...
    if step == 1 && P.currentcell < P.nplots
        hold off;
...
% add to this list any parameters shown manually with DATA.show
    ...
    for ij = 1:length(ids);
        ...
        elseif DATA.autocut & (size(Clusters) < nb | size(Clusters{nb},2) < DATA.probe ...
            ...
            DATA = CombineAutoCut(DATA,DATA.expid(j),j);
            Clusters{nb} = DATA.Expts{DATA.expid(j)}.Cluster;
            recount = 1;
        else
            recount = 0;
...
%            DATA.probes = Probes;
    ...
    if DATA.bysuffix
        DATA.state.online = 0;
        if isfield(DATA,'CellDetails') && isfield(DATA.CellDetails,'excludetrialids')
        ...
        for j = 1:length(DATA.Expts)
            if isfield(DATA,'Clusterfile') && length(DATA.Clusterfile) >= j
                if isfield(DATA.Clusterfile{j},'excludetrialids')
                    [DATA.Clusterfile{j}.excludetrialids] = deal([]);
                end
...
elseif isfield(DATA,'AllClusters')
    if iscell(DATA.AllClusters)
        DATA = CheckClusterLoaded(DATA, expid, DATA.probe);
        if DATA.listbycell
...
if isfield(DATA,'CellList') && size(DATA.CellList,1) >= j
...
function PlotAllCells(a,b, plottype)
   ...
   elseif strncmp(plottype,'load',4)
       outname = [DATA.Expt.Header.fileprefix '.Cells.' DATA.Expt.Header.expname '.mat'];
       load(outname);
   ...
   for j = 1:length(Expts)
       ...
       elseif isfield(P,'bestdelay')
           ...
           P(1).var(1) = std(sqrt(P.means(:)));
           vars = [];
           for k = 1:length(P.counts(:))
               vars(k) = var(sqrt(P.counts{k}));
           end
