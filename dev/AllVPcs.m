...
%new data but figure is already up, 
    ...
    if isfield(Vall,'exptno')
        ...
        if isfield(DATA,'toplevel') && isfigure(DATA.toplevel) && myisappdata(DATA,'Expts')
            if verbose > 1
...
%    DATA.currentcluster = 1;
    ...
    elseif length(DataClusters) < DATA.probelist(DATA.probe(1))
        wasauto = 2;
    elseif DataClusters{DATA.probelist(DATA.probe(1))}.auto == 1
        wasauto = 1;
    else
        wasauto = 0;
    end
...
function ProbeSelector(DATA)
   ...
   if isnew
       ...
       w = 1./nc;
       h = 1./nr;
       X = uibuttongroup(F,'SelectionChangeFcn',{@SelectProbe, 0});
