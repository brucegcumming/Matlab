...
if isfield(DATA,'AllClusters') && DATA.bysuffix == 0
    return;
end
if ispk
    ...
    if DATA.probe < 100
        if isfield(DATA,'AllClusters')
            cx = DATA.AllClusters{DATA.currentexpt}(DATA.probe).cx(ispk);
            cy = DATA.AllClusters{DATA.currentexpt}(DATA.probe).cy(ispk);
        else
...
function nc = MaxSpikeCode(DATA, expspks)
    ...
    if isfield(DATA,'AllClusters')
        if iscell(DATA.AllClusters)
        nc = max(DATA.AllClusters{DATA.currentexpt}(DATA.probe).codes(expspks,ctype));
        else
...
elseif isfield(DATA,'AllClusters') && iscell(DATA.AllClusters)
    ...
    if length(DATA.AllClusters{nexp}) < probe
        ispk = [];
    else
...
while j <= length(varargin)
    if strncmpi(varargin{j},'xyonly',6)
        xyonly = 1;
    end
    j = j+1;
end
...
for j = 1:length(DATA.Expts);
    ...
    if   DATA.listbycell == 1 
        count=0;
        for k=1:length(DATA.AllClusters)
            if ~isempty(DATA.AllClusters{k})
                count=count+1;
                expy(count)=k;
            end
        end
        pid = find(DATA.CellList(expy(j),:) == DATA.probe) ;
        if length(pid) ~= 1
            ok = 0;
        end
    end
...
function PlotRateSequences(DATA, varargin)
   ...
   colors = mycolors;
    ...
    for j = 1:length(DATA.AllExpts)
        E = DATA.AllExpts{j};
        h(j)  = plot(find(ismember(AllTrials,[E.Trials.Trial])), [E.Trials.count],'o','color',colors{j});
...
function PlotAllCells(a,b, plottype)
   ...
   for j = 1:length(Expts)
       ...
       if isfield(Expts{j}.Header,'cell')
   ...
   for j = 1:length(Expts)
       ...
       if isfield(P,'bestdelay')
           ...
           elseif strmatch(plottype,{'blank' 'blankim'})
               V = P.sdfs.extras{1}.sdf(ts:end);
               scale = 1./median(V);
               plot(V.*scale,'-','color',colors{j});
               im(did(j),:,yi) = V.*scale;
           else
               plot(P.x(:,t),P.y(:,yi,t).*scale,'o-','color',colors{j});
               im(did(j),:,1) = Expts{j}.plotres.y(:,1,t).*scale;
           end
       else
           nmin = 1+ median(Expts{j}.plotres(1).n(:))/3;
           aid = find(Expts{j}.plotres(1).n(:) >= nmin);
           if size(Expts{j}.plotres(1).n,2) >= yi %some cells might be missing some values
           id = find(Expts{j}.plotres(1).n(:,yi) >= nmin);
           if strmatch(plottype,{'normmax' 'imagemax'})
               scale = 1./max(Expts{j}.plotres(1).means(aid));
           elseif strmatch(plottype,{'normmean' 'imagemean'})
               scale = 1./mean(Expts{j}.plotres(1).means(aid));
           else
               scale = 1;
           end
           h(j) = plot(Expts{j}.plotres(1).x(id,yi),Expts{j}.plotres(1).means(id,yi).*scale,'o-','color',colors{j},'linestyle',lines{yi});
           labels{j} = sprintf('C%d',Expts{j}.Header.cellnumber);
           if strmatch(plottype,{'imagemax' 'imagemean'  })
               im(did(j),1:length(id),yi) = Expts{j}.plotres(1).means(id,yi).*scale;
           end
           end
       end
       hold on;
   ...
   if strmatch(plottype,{'imagemax' 'imagemean' 'varim' 'blankim'},'exact')
       for j =  1:ny
           subplot(1,ny,j);
           hold off;
           imagesc(squeeze(im(:,:,j)));
           caxis(minmax(im(:)));
           xl = get(gca,'xlim');
           if sum(cells ==0) == 0
               set(gca,'ytick',[]);
            ...
            if ~isempty(id)
                if listonly
                    ...
                    if lfponly > 0
                        ...
                        DATA.spikelist = spikelist;
                        SetClusterCheck(DATA);
                        if p == 1 && DATA.bysuffix
                            CombineAllCells(a, DATA);
                        end
                        if p ==1  && lfponly == 0 && (exist('LFP') || DATA.bysuffix)
                            DATA.Expt = Expt;
                            combine('savelfp',DATA,LFP);
                        end
                        %                CombineAll(a,DATA);
...
for j = clusters
            if isfield(DATA,'AllClusters')
                if iscell(DATA.AllClusters)
                    id = find(DATA.AllClusters{DATA.currentexpt}(DATA.probe).codes(expspks,ctype) == j-1);
                else
                    id = find(DATA.AllClusters(DATA.probe).codes(expspks,ctype) == j-1);
                end
            elseif isfield(DATA,'AllSpikes')
...
function dp = CalcClusterdp(DATA, cl)  %old method using ROC of radial distances.
    ...
    elseif isfield(DATA,'AllClusters')
        id = find(DATA.AllClusters{DATA.currentexpt}(DATA.probe).codes(expspks,2) == cl-1);
        nid = find(DATA.AllClusters{DATA.currentexpt}(DATA.probe).codes(expspks,2) ~= cl-1);
    else
...
function DATA = FinishXYPlot(DATA)
    ...
    elseif isfield(DATA,'AllClusters') && iscell(DATA.AllClusters)
        cluster = DATA.AllClusters{DATA.currentexpt}(DATA.probe);
        title(sprintf('%s: C%d Sfx%d',expname,c,cluster.suffix));
    else
