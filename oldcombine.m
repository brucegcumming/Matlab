...
if isempty(it)
    ...
    if ~isfield(DATA,'toplevel') %%Error 
        if strcmp(name,'quit')
            ...
            fprintf('Cant Find combiune window - quitting. run combine(''quit'') to close window\n');
        end
        return;
    end
...
%do recombine at end = so dont set recmone > 0
       recombine = 0;
...
while j <= length(varargin)
    ...
    elseif strncmpi(varargin{j},'loadclusters',8)
       ...
       set(DATA.toplevel,'UserData',DATA);
 elseif strncmpi(varargin{j},'recombinemucells',12)
    out = ReCombineAll(DATA,0,'mucells',recombinenames);
...
function AllCellKeyPressed(src,ks, op)
     ...
     if gcf ~= DATA.fig.CombinerAllExpts;
         setappdata(gcf,'ParentFigure',DATA.toplevel);
         set(gcf,'KeyPressFcn',@AllCellKeyPressed);
     end
...
function [Expt, DATA, plotres] = CombinePlot(DATA, dlgs, varargin)
    ...
    if DATA.listbycell
        if DATA.listbycell == 2
            [eid, cid] = FindNoCell(DATA, DATA.probe, DATA.expid);
        else
                    ...
                    for c = 1:size(DATA.CellDetails.excludetrials,3)
                        if ~isempty(DATA.CellDetails.excludetrials{j,k,c})
                            ...
                            if c > length(DATA.AllClusters{eid}(k).excludetrialids)
                                ...
                                DATA.AllClusters{cid}(k).excludetrialids{c} = union(DATA.AllClusters{eid}(k).excludetrialids{c}(:)',DATA.CellDetails.excludetrials{j,k,c}(:)');
                            end
                            end
                        end
                    end
...
if isfield(DATA,'CellList') 
    ...
    if size(DATA.CellList,1) < j       
        ...
        warndlg(['Number of Expts in CellList does not match' str],'Expt List mismatch');
    end
...
nmin = 1+ median(P(1).n(:))/3;
       ...
       AllCellRes(j,:) = P;
           ...
           for k = 1:size(P)
               ...
               AllCellRes(j,k) = CopyFields(AllCellRes(j,k),P(k));
           end
       end
   end
   end
   ...
   SetCellChooser(DATA);
    ...
    subplot('Position' ,[x y pw ph]);
    hold off;
    DrawXYPlot(DATA, DATA.Expts{xi}.gui.spks,'usefigure');
    xl = get(gca,'Xlim');
    yl = get(gca,'ylim');
...
if ~isempty(recombinenames)
    ...
    for j = 1:length(recombinenames)
        id = find(strncmp(recombinenames{j},DATA.explabel(2:end),length(recombinenames{j})));
        if isempty(id)
            ...
            if ~isempty(id)
                if listonly
                    ...
                    if lfponly > 0
                        ...
                        SetClusterCheck(DATA);
                        if p == 1 && docells
                            out{j} = CombineAllCells(a, DATA, cargs{:});
                        end
...
function [DATA, D] = CombineLoadCluster(DATA, eid, pid)
    if isfield(DATA,'suffixlist')
    ...
    if isfield(DATA.CellDetails,'trialids')
        DATA.CellDetails = CheckExptClusters(Clusters,DATA.Expts,DATA.CellDetails);
...
ctype = 2;
PlaySpikes(DATA, DATA.currentexpt(1), varargin{:}, 'xyonly');
...
function DATA = DrawXYPlot(DATA, expspks, varargin)
    ...
    if isfield(DATA,'AllSpikes')
        ...
        if length(expspks)
           PlotSpikeXYClusters(DATA, expspks, 1:DATA.nclusters+1,varargin{:});
        end
    else
        if length(expspks) < 10 
            expspks = DATA.Expts{DATA.currentexpt(1)}.gui.spks;
        ...
        if DATA.densityplot
            ...
            PlotSpikeXYClusters(DATA,expspks,1:nc+1,varargin{:});
        end
    end
    hold on;
    DATA = DrawClusters(DATA, DATA.cluster, 0);
