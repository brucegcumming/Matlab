function fig = plotonegrid(file, interp, gaussfit , greyscale, fitfft, theplot, plottype)

% theplot controls plot type. 
% 5 is plot3
% 3 is vhplot
%
%
%plotonegrid(file,0,0,0,0,5,1)
%if addfit = 0 shows contours by interpolatio
%if addfit = 2 shows no contours
addfit = plottype.addfit;
final = plottype.final;
showori = plottype.showori;
ocolor = [0.0 1.0 1.0];
hold off
...
if ~(interp)
  xoff = xinc/2 * sin(orirad) + yinc/2 * cos(orirad);
  yoff = yinc/2 * sin(orirad) - xinc/2 * cos(orirad);
  DX = DX - xoff;
  DY = DY - yoff;
end

dxinc = DX(orows,cols) - DX(orows-1,cols);
DXall = [DX;(DX(orows,:)+dxinc)];
dxinc = DX(orows,cols) - DX(orows,cols-1);
DXall = [DXall (DXall(:,cols) + dxinc)];
dyinc = DY(orows,cols) - DY(orows-1,cols);
DYall = [DY;(DY(orows,:)+dyinc)];
dyinc = DY(orows,cols) - DY(orows,cols-1);
DYall = [DYall DYall(:,cols)+dyinc];
Zall = [Z;Z(orows,:)];
Zall = [Zall Zall(:,cols)];
...
% 1 2   3  4  5   6    7   8   9
sumfile = strrep(file,'.oxp','.sum');
index = max(findstr(sumfile,'/'))
sumfile = strrep(sumfile,sumfile(1:index),'./summaries/')
[sumlabels, sumvals] = textread(sumfile,'%s %f');
...
minx = min(X,[],2);
xinc = (maxx(1)-minx(1))/(orows-1);
xi = linspace(minx(1)-xinc, maxx(1), 40);
yc = (maxy(1)+miny(1))/2; 
xc = (maxx(1)+minx(1))/2;
...
if interp
  ...
  if (theplot == 1)
    ...
    hold on;
    if(addfit)
      zirange = [ max(max(zfit)), min(min(zfit))];
    else
  ...
  if(theplot == 5)
    ...
    set(gca,'xgrid','on','ygrid','on','zgrid','on', 'Xlim', xrange, ...
	    ...
	    2,'fontsize',14,'view',[35 45]);
    set(gca,'pos',[0.25 0.25 0.6 0.75]);
    ...
    ylabel({'Parallel Disparity','  (degrees)'},'Rotation',42);
    zlabel('Firing rate (spikes.sec^{-1})');
  end %if theplot == 5
...
if(theplot == 3)
  ...
  ylabel('Vertical Disparity (degrees)');
  xlabel('Horizontal Disparity (degrees)');
  minx = min(min(DX));
  maxx = max(max(DX));
  miny = min(min(DY));
  maxy = max(max(DY));
  ...
  if interp
    ...
    fig = pcolor(DXall,DYall,Zall);
    hold on
    
    xo = xc;
    yo = yc;
    if(orirad > pi/8 & orirad < 3 * pi/8)
      xo = xc;
      yo = yc+vw/2;
    end
    set(gca,'fontsize',18);
    if ~(final)
      title(sprintf('Disparity Orientation %.0f Ori %.0f',dori,ori));
    if (findstr(file,'ruf011'))
      xo -0.14;
      yo = 0.25;
      tca = text(xo,yo,{'orientation tuning',' (monocular)'},'Rotation',-35);
    elseif (findstr(file,'duf025'))
      xo = -0.32;
      yo = 0.55;
      tca = text(xo,yo,{'orientation tuning',' (monocular)'});
    else
      tca = text(xo,yo,{'orientation tuning',' (monocular)'});
    end
    set(tca,'color',ocolor,'fontsize',18);
    end
    caxis([min(zrange),max(zrange)]);
    if(doshade)
      shading('interp');
  ...
  hold on
    if(addfit == 1)
      ...
      [C , h] = contour(xxi,yyi,zfit,8);
      %    fprintf('X %.2f Y %.2f -> %.2f %.2f\n',xinc,yinc,xoff,yoff);
    elseif(addfit == 2)
      ;
    elsif(~interp)
  ...
  h = colorbar;
  get(h,'Position');
  set(h,'fontsize',18,'Position',[0.78,0.11,0.058,0.815]);
  xr = get(gca,'Xlim');
  yr = get(gca,'Ylim');
%  text(xr(2)+(xr(2)-xr(1))/3,(yr(2)+yr(1))/2,'Spikes/sec','fontsize',18,'Rotation',-90,'HorizontalAlignment','center');
  text(xr(2),yr(1)-(yr(2) - yr(1))/10,'Spikes/sec','fontsize',18);
  %rotate(fig,[0 90],45);
  ...
  %plot(OMARKX, OMARKY, 'w');
    if(length(sumvals) > 1)
      x = sumvals(strmatch('MAXINT_HONLY ',sumlabels))
      plot(x,0, 'xb');
    end
    
  
  otfile = strrep(file,'rds.OxPD.oxp','grating.prefOT.ma');
  if exist(otfile) & showori
    OR =  dlmread(otfile,' ');
    A = OR(:,2);
    R = OR(:,1) * pi/180.0;
    [XX, YY] = pol2cart(R,A);
    ...
    if(length(sumvals) > 1)
      xcntr = sumvals(strmatch('MAXINT_X ',sumlabels));
      ycntr = sumvals(strmatch('MAXINT_Y ',sumlabels));
      XX = (XX .* scale) + xcntr;
      YY = (YY .* scale) + ycntr;
    else
      XX = (XX .* scale) + xc+xoff;
    ...
    if(~final)
      plot([xcntr xcntr+max(A)*cos(orirad)*scale], [ycntr ycntr+ ...
		    max(A)*sin(orirad)*scale],'color', ocolor, 'linewidth', 2);
      plot([xcntr-max(A)*cos(dorirad)*scale xcntr+max(A)*cos(dorirad)*scale], [ycntr-max(A)*sin(dorirad)*scale ycntr+ ...
		    max(A)*sin(dorirad)*scale],'color', 'r', 'linewidth', 2);
    end
  ...
  hold off
% set nopcolor to show just the contour and the  orientation tuning  
  nopcolor = 0;
  if(nopcolor)
    delete(fig);
    set(gca,'color','none');
