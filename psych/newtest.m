...
for j = 1:length(summ)
    for k = 1:length(summ(j).data)
        resumm(j).data(k).p = binornd(resumm(j).data(k).n,resumm(j).data(k).p,nresample,1);    
    end
end
...
for sd = unique([sumData.sd])
  ...
  for x = unique([sdtrials{j}.x])
      ...
      tdata(ndat).sd = sd;
      tdata(ndat).expno = j;
        tdata(ndat).resp = length(find([tdata(ndat).ups] >= 1));
        tdata(ndat).p = tdata(ndat).resp / tdata(ndat).n;
       tdata(ndat).n = length([tdata(ndat).ups]);
        bredata(ndat).sd = sd;
        bredata(ndat).x = x;
        bredata(ndat).xo = mean([sdtrials{j}(idx).xo]);
        tdata(ndat).xo = bredata(ndat).xo;
