function response = Run_one_sine(stim_disparity, rf_disparity, filtersize, sd, freq, stimcycles)
%
%	response = Run_one_sine(stim_disparity, rf_disparity, filtersize, sd, freq, stimcycles)
%
%	returns 4 by array of responses of 4 rf's to a sine stimulus
...
%
%	NB at the moment runs only one position (phase) of the stim - complex response should be same regardless

% set appropriate rf's
	[rf_left_cos, rf_left_sin, rf_right_cos, rf_right_sin] = Setup_gabor_rfs(rf_disparity, filtersize, sd, freq);

% calculate response of rf's sine stimulus
	for phase = 1:filtersize
	    [leftstim, rightstim] = Stim_sine(stim_disparity, filtersize, stimcycles, phase);
