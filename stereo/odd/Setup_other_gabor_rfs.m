function [rf_left_cos, rf_left_sin, rf_right_cos, rf_right_sin] = Setup_other_gabor_rfs(rf_disparity, filtersize, sd, cycles)
%
...
% make and scale gabor filters (equal area with change of sd)
	...
	rf_left_sin = Gabor(centre, sd, cycles, (1.5 * pi));
...
% now make right eye with appropriate disparity
	rf_right_cos = Gabor(centre + rf_disparity, sd, cycles, pi+ ...
			     adj);
	rf_right_sin = Gabor(centre + rf_disparity, sd, cycles, (pi/2) + adj);
