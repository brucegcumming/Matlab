...
% call with 'psychflip'
      ...
      if ~isempty(idx) & ~isempty(uidx)
          ...
          elseif strmatch(Expt.Stimvals.e2,'ob') && ~strcmp(type,'cvsign')
              id = find([Expt.Trials.ob] > 120);
...
for nc = 1:length(cvals)
    ...
    for ie = 1:length(bvals);
        ...
        result.colors{ie} = colors{ie+addn};
    ...
    if isfield(Expt.Trials,'RespDir')
        ...
        if showplot & ~mksdf & isfield(result,'x') & ~plotsimple & ie <= size(result.x,2)
            ...
            if fillsymbols & nc == 1
                set(allh,'MarkerFaceColor',colors{ie+addn});
            end
            hold on;
            if getmod
            ...
            set(er,'color',colors{ie+addn},'linestyle',linestyle{nc});
...
if isfield(result,'n') %% empty if no data
...
if isfield(result,'x') %% empty if no data
