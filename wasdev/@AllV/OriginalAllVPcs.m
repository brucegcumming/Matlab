...
tt = TimeMark(tt,'Created AllVoltages',DATA.profiling);
...
elseif verbose
    fprintf('E%dP%d Made AllVoltages  %d spikes at %s (%.2f)\n',DATA.exptno,ProbeNumber(DATA),size(AllVoltages,3),datestr(now,'HHMM:ss'),mytoc(tt(1).time));
end
tt = TimeMark(tt,'Built AllVoltages',DATA.profiling);
...
if length(tryall) > 1 % when reapplying, just use CSD/Dvdt as before
    ...
    for j = 1:length(tryall)
        ...
        DATA.dvdt = dvdts(j); 
        DATA.csd = csds(j);
        [Cs{j}, Evec{j}, pcs{j}, dips{j}, chsppks{j}, errs, fits{j}] = CalcPCs(DATA,AllVoltages,nprobepc);
        DATA.errs = {DATA.errs{:} errs{:}};
        else
            dips{j} = 0;
        end
...
%    DATA.chspk = chspk;
    ...
    if recluster %no need to calcuate GMfits etc
        [C, res.Evec, res.pcs, dip, chspk, errs] = CalcPCs(DATA,AllVoltages,nprobepc,'nofit');
        DATA.errs = {DATA.errs{:} errs{:}};
        ...
        [C, res.Evec, res.pcs, dip, chspk, errs, DATA.pcfit] = CalcPCs(DATA,AllVoltages,nprobepc);
        res.dipvals = dip;
    end
DATA.pcs = res.pcs;
DATA.Evec = res.Evec;
...
F = SetFigure(DATA.tag.top,DATA);
DATA.toplevel = F;
DATA =  mysetappdata(DATA,'AllVoltages',AllVoltages);
...
%line, set it in the cluster 
        ...
        if (ismember(recluster,[1 2]) || forceevec) && clusterspace == 1
            ...
            if isfield(DATA.cluster,'Evec') && (recluster == 2 || forceevec) %recalssify used old PCs. 
                ...
                [a,DATA.Evec, DATA.pcs] = CalcPCs(DATA, AllVoltages, DATA.nprobepc, DATA.cluster.Evec);
                DATA.cluster.forceevec = 1;
            elseif loadfromspikes == 0
%When do we need to do this?  SHould have been done above?  ?? for rclassify                
            [a,DATA.Evec, DATA.pcs(DATA.uid,:)] = CalcPCs(DATA, AllVoltages, DATA.nprobepc);
...
function DATA = ReClassify(DATA, varargin)
    newbound = 0;
    fittemplate = 0;
    AllVoltages = mygetappdata(DATA,'AllVoltages');
...
function DATA = NextPCs(DATA)
    ...
    [C, DATA.Evec, pcs, dip, chspk, errs, DATA.pcfit] = CalcPCs(DATA,AllVoltages,DATA.nprobepc);
...
spts = DATA.spts;
for j = 1:length(id);
    plot([DATA.t(id(j)):DATA.t(id(j))+length(spts)-1]+spts(1),AllVoltages(ispk(p),:,id(j))+y,'r');
end
nid = DATA.nid;
for j = 1:length(nid);
    plot([DATA.t(nid(j)):DATA.t(nid(j))+length(spts)-1]+spts(1),AllVoltages(ispk(p),:,nid(j))+y,'g');
end
y = y + max(V(ispk(p),:));
end
...
% not the index to the current AllVoltages Array.
...
function SaveSpikes(DATA, id, name)
    ...
    if DATA.saveallspikes
        ...
        if ~strcmp(name,xname) && size(AllVoltages,1) > 1
...
function [E, cluster] = CutAndSave(DATA, varargin)
    ...
    if strcmp(DATA.autocutmode,'james')
        ...
        [AllVoltages, DATA] = BuildAllV(DATA, D.spk_inds(D.uids)', D.params.spk_pts);
        outname = ClusterFile(DATA.name,DATA.Expt,'auto','subdir',DATA.clustersubdir);
...
%artifacts, so exclude spikes after trial end when making the cut
   AllVoltages = mygetappdata(DATA,'AllVoltages');
    ...
    try
    [V, distance(3), a] = GMfit(squeeze(AllVoltages(p,DATA.vspace,uid))',nc,1);
    catch
...
function Spikes = MakeJamesSpikes(DATA)
    V = mygetappdata(DATA,'AllVoltages');
    V = V(DATA.chspk,:,:);
...
if reapply || retrigger
    ...
    [AllVoltages, DATA] = BuildAllV(DATA, vid', details.params.spk_pts);
    DATA = SetPCs(DATA,0);
...
%    E.pos = [crit min(xy(:,2)) crit max(xy(:,2))];
    ...
    p = DATA.vpts;
    for j =1:length(p)
        [as(j+n),bs(j+n),c] = BestAngle(AllVoltages(p(j,1),p(j,2),:),AllVoltages(p(j,3),p(j,4),:),2);
...
elseif ismember(fcn,[1 2 3])
    [C, DATA.Evec, DATA.pcs, DATA.dipvals, DATA.chspk] = CalcPCs(DATA,AllVoltages,fcn-1);
    DATA = ReplotPCs(DATA,[]);
    set(DATA.toplevel,'UserData',DATA);
elseif fcn == 4
    DATA.dvdt = ~DATA.dvdt;
...
if strncmp(fcn,'refit',5)
    ...
    DATA = RunGMMFit(DATA,C,'replot','space',Sf);    
...
elseif strncmp(fcn,'add',3)
    ...
    DATA = SetEllipseDrawing(DATA, 0,'cluster',1,'gmmadd');
elseif strncmp(fcn,'split',5)
    setappdata(DATA.toplevel,'LastGMM',DATA.cluster.jamescluster);
    cn = sscanf(fcn,'split%f');
    DATA = SetEllipseDrawing(DATA, 1,'cluster',cn,'gmmsplit');
elseif strncmp(fcn,'merge',4)
    setappdata(DATA.toplevel,'LastGMM',DATA.cluster.jamescluster);
    DATA = SetEllipseDrawing(DATA, 1,'cluster',1,'gmmmerge');
...
function [x,y] = GetClusterXYData(DATA, p)
    ...
    elseif usespace == 2 %voltage pairs
        AllVoltages = GetDataStruct(DATA, 'AllVoltages');
        x = AllVoltages(p(1),p(2),DATA.uid);
        y = AllVoltages(p(3),p(4),DATA.uid);
...
%        uimenu(sm,'Label','&Good','Callback',{@PCCluster, 'markgood'},'Tag','MarkGood');
        ...
        uimenu(sm,'Label','&Plot GMM components','Callback',{@GMMmenu, 'setplot'});
        uimenu(sm,'Label','Split C&1','Callback',{@GMMmenu, 'split1'});
        uimenu(sm,'Label','Split C&2','Callback',{@GMMmenu, 'split2'});
        uimenu(sm,'Label','Split C&3','Callback',{@GMMmenu, 'split3'});
        uimenu(sm,'Label','Split C&4','Callback',{@GMMmenu, 'split4'});
        uimenu(sm,'Label','&Merge','Callback',{@GMMmenu, 'merge'});
        uimenu(sm,'Label','&Add','Callback',{@GMMmenu, 'add'});
        uimenu(sm,'Label','&Undo','Callback',{@GMMmenu, 'undo'});
        sm = uimenu(sm,'Label','&Refit');
        uimenu(sm,'Label','&Current Space','Callback',{@GMMmenu, 'refit current'});
        uimenu(sm,'Label','&Template Space','Callback',{@GMMmenu, 'refit template'});
...
%if quck ==2, do the 1d fit, so we can see fit quality             
        ...
        elseif E.space(1) == 2
            AllVoltages = GetDataStruct(DATA, 'AllVoltages');
            if DATA.verbose > 1
                fprintf('%s AllVoltages %s(%d)\n',IDStr(DATA),sprintf('%d ',size(AllVoltages)),DATA.toplevel);
            end
            xy(DATA.uid,1) = AllVoltages(p(1),p(2),DATA.uid);
            xy(DATA.uid,2) = AllVoltages(p(3),p(4),DATA.uid);
            cluster.space = [2 p(1:4)];
...
for j = 1:length(oT)
    ...
    if DATA.usestdtemplates
        ...
        Scores(1,1,:) = MeanSpike(1,:) * squeeze(AllVoltages(j,:,:) - meanV);
        Scores(1,2,:) = MeanSpike(2,:) * squeeze(AllVoltages(j,:,:) - meanV);
        Scores(1,3,:) = squeeze(diff(DATA.StdTemplate(1,:),1,2)) * squeeze(diff(AllVoltages(j,:,:),1,2));
        Scores(1,4,:) = squeeze(diff(DATA.StdTemplate(2,:),1,2)) * squeeze(diff(AllVoltages(j,:,:),1,2));
        Scores(1,5,:) = squeeze(diff(DATA.StdTemplate(2,:),1,2)) * squeeze(diff(AllVoltages(j,:,:),1,2));
        return;
    end
...
%does meanV need to know about vid?    
    ...
    if length(DATA.chspk) == 1
        ...
        Scores(1,6,:) = MeanSpike.mu * squeeze(AllVoltages(DATA.chspk,:,:) - meanV);;
        Scores(1,7,:) = DATA.rV;
...
for j = 1:size(plots,1)
    ...
    elseif ismember(DATA.plottype,[2 6 8 9])
        ...
        elseif ismember(DATA.plottype,[13]) %james autocut
            PlotFeatures(DATA,plots(j,1),plots(j,2),DATA.clplot,clid,DATA.colors,C, 'fixrange');
            showclustermeans = 0;
...
if mode == 2  || DATA.elmousept.shape  ==1 
    if strncmp(DATA.elmousept.type,'gmm',3)
        GMMButton(DATA, DATA.elmousept,DATA.elmousept.type);
    else
