...
charstrs = {'Covariate'  'hxtype' 'cx' 'adapter' 'exp' 'et' 'e2' 'e3' 'exptlabel' 'exptvars' 'imprefix' 'stimtag' 've' 'Bs'};
extrastrs = {'StartDepth' 'Electrode' 'Write' ' Electrode' 'Experiment' 'Off at' 'CLOOP' 'cm=' ...
    ...
    'expname' 'immode' ...
    'seof' '/local' 'imve' 'Sa:' 'op' 'annTyp' 'uf' 'lo' 'vve' 'testflag' 'Hemisphere' 'Reopened' 'monitor' 'RightHemi' 'ui'};
...
Trials = AddNetFileData(name, Trials);
...
function T = AddNetFileData(name, T)
...
for j = 1:length(cf)
    if isfield(T,cf{j}) && ~iscell(cf{j})
        x = T.(cf{j});
        T.(cf{j}) = {};
        for k = 1:length(x) %first convert to cell
            T.(cf{j}){k} = x(k);
        end
    end
    for k = 1:length(nid)
        T.(cf{j}){sid(k)} = bT(nid(k)).(cf{j});
    end
end
