...
while j <= length(varargin)
    ...
    elseif isfield(varargin{j},'toplevel')
        DATA = varargin{j};
    elseif isempty(varargin{j})
        
    elseif strmatch(varargin{j},{'dips' 'probequality' 'quality' 'mahaldip' 'bestspace' 'probesd' 'nclusters'})
        plottype = varargin{j};
...
elseif strncmp(plottype,'dips',4) || strncmp(plottype,'quality',4)
    ...
    result.dips = dips;
elseif strncmp(plottype,'nclusters',7)
    hold off; 
    imagesc(DATA.nclusters,'buttondownfcn',{@PC.HitImage, 1});
