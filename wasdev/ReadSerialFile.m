...
%can be much later on Late resp trials
    ...
    if isempty(id)
        id = find(idid < sqid(j));
        if verbose && idid(id(end)) > sqid(1)
            fprintf('Missing Response String for line %d Trial %d %s Trial %d\n',sqid(j),j,txt(idid(id(end)),:));
        end
        id = find(pid < sqid(j));
        if isempty(id)
            d = 100;
...
%    end
    ...
    if scores(t) == 0 && trialmode == 1  %fixation trial for dx:  Take it
        Trials(j).result = 1;
        Trials(j).RespDir = 0;
    elseif scores(t) == -3 || scores(t) == -2 || (length(seedseq{j}) > 250 && nfs(j) < 250)
        Trials(j).result = -1;
        Trials(j).RespDir = 0;
...
for j = 1:length(exid)
    ...
    if length(id)
        ...
        fprintf('Ids missing for expt %d\n',j);
    end
    if length(tid) == 1
