...
function DATA = ReadFinishedDir(DATA, name, varargin)
    ...
    for nexp = 1:length(Expts)
       ...
       eids(nexp) = Expts{nexp}.Header.suffix;
    end
    DATA.suffixlist = eids;
    
    DATA.Expts(eids) = Expts;
...
%when combineing across suffixes make sure .Trial is unique and monotonic. This
% way .id can be left alone as it matches the serial output
                if isempty(Trialids)
                    trialoffset = 0;
                else
                    trialoffset = max(Trialids) - Expts{nexp}.Trials(1).Trial;
                end
                Expts{nexp}.Header.trialoffset = trialoffset;
                for k = 1:length(Expts{nexp}.Trials)
                    news(k) = Expts{nexp}.Trials(k).Start(1);
% Don't remove this. Its iimportant for making .Trial consecutive, and used for
% Header.BlockStart
            ...
            if isfield(Clusters{j},'xy')
                ...
                if isfield(Clusters{j},'errs')
                    ...
                    for e = 1:length(Clusters{j}.errs)
                        ...
                        fprintf('%s\n',e);
                        if strncmp(e,'Missing Trials',15)
