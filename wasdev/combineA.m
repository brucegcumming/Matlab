    ...
    if strncmp(DATA.filetype,'Grid',4) && playspk 
        if DATA.usenev
           DATA = ReadGridFile(DATA);
            ...
            DATA = CheckClusterLoaded(DATA, DATA.currentexpt);
        end
...
function DATA = OldReadGridFile(DATA)
...
function AllCellKeyPressed(src,ks, op)
     ...
     if strcmp(ks.Key,'downarrow') && P.currentcell < P.nplots
         ...
         hold off;
         PlotResult(AllCellRes(P.currentcell));
...
f = {'mahal' 'crit' 'ctime' 'excludetrialids' 'clustersign'};
for j = 1:length(f)
    if isfield(C,f{j})
        Cluster.(f{j}) = C.(f{j});
    end
...
function [Expt, DATA, plotres] = CombinePlot(DATA, dlgs, varargin)
    ...
    if DATA.extype ==1
        msgbox('You must select an Expt type before combining','Selection Error','OK');
        Expt = [];
        return;
    end
    ...
    for ij = 1:length(ids);
        ...
        if isfield(DATA,'AllClusters')
            ...
            if isfield(Clusters{nb},'excludetrialids')
                excludeids = cat(2,excludeids,Clusters{nb}.excludetrialids);
            end
...
%Expt.trials.excluded has excluded trias fomr AllVPcs in AllClusters
    id = find(~ismember([Expt.Trials.id],excludeids) & [Expt.Trials.excluded] == 0);
    Expt.Trials = Expt.Trials(id);
    id = find([Expt.Trials.excluded] ==1);
    Expt.Header.excludeids = union(excludeids,[Expt.Trials(id).id]);
...
if DATA.bysuffix
    ...
    for j = 1:length(DATA.monkeys)
        if strfind(name,['/' DATA.monkeys{j}]);
            DATA.monkey = DATA.monkeys{j};
        end
    end
...
%            DATA.probes = Probes;
    ...
    if DATA.bysuffix
        ...
        if isfield(DATA,'CellDetails')
            for j = 1:size(DATA.CellDetails.exptids,1)
                eid = floor(DATA.CellDetails.exptids(j));
                for k = 1:size(DATA.CellDetails.excludetrials,2)
                    DATA.AllClusters{eid}(k).excludetrialids = union(DATA.AllClusters{eid}(k).excludetrialids,DATA.CellDetails.excludetrials{j,k});
                end
            end
        end
        for j = 1:length(DATA.Expts)
            if isfield(DATA,'Clusterfile') && length(DATA.Clusterfile) >= j
                if isfield(DATA.Clusterfile{j},'FullVData')
                    ...
                    if length(xcl)
                        for k = 1:length(DATA.AllClusters{j})
                            DATA.AllClusters{j}(k).excludetrialids =  [DATA.AllClusters{j}(k).excludetrialids xcl];
                        end
                    end
                end
            end
...
elseif isfield(DATA,'AllClusters')
    if iscell(DATA.AllClusters)
        DATA = CheckClusterLoaded(DATA, expid);
        if DATA.listbycell
            ...
            Spks = DATA.AllClusters{expid}(DATA.probe);
...
if length(id)
    DATA.AllData.Spikes.codes(ids{1}(id),ctypr) = 0;
end
end
...
function DATA = oldGetDataFromFig(a) %now a spearte function
...
nmin = 1+ median(P(1).n(:))/3;
           ...
           if size(P(1).n,2) >= yi %some cells might be missing some values
               ...
               xv = P(1).x(id,yi,:);
               h(j) = plot(xv,P(1).means(id,yi,:).*scale,'o-','color',colors{j},'linestyle',lines{yi});
               labels{j} = sprintf('C%d',Expts{j}.Header.cellnumber);
               if strmatch(plottype,{'imagemax' 'imagemean'  })
                   iid = find(ismember(allx, xv));
                   im(row,iid,yi) = P(1).means(id,yi,1).*scale;
               end
           end
           allxv = [allxv;  P.x(:,1)];
           if strmatch(P.type{1},{'Pp' 'Op'})
               for t = 1:length(P.ids);
                   if isfield(Expts{j}.Trials,'xo')
                   P.xpos(t) = mean([Expts{j}.Trials(P.ids{t}).xo]);
                   end
                   if isfield(Expts{j}.Trials,'yo')
                   P.ypos(t) = mean([Expts{j}.Trials(P.ids{t}).yo]);
                   end
               end
           end
       end
       hold on;
       if isfield(P, 'fit') && isfield(P.fit,'xv')
...
function DATA = CalcMeanSpike(DATA,expid)
    ...
    if isempty(Spks) %Can happen if bysuffix
        return;
    end
...
docells = 1;
while j <= length(varargin)
    if strncmpi(varargin{j},'cells',5)
        docells = 1;
...
if strfind(DATA.outname,'image.ORBW')
 ...
 if makemu
     for p = 1:size(DATA.CellList,2)
         ...
         if nex >= length(DATA.expid)/2
             domu = [domu p];
         end
     end
 end
for j = 1:(length(cells)+length(domu))
    if j > length(cells)
    ...
    [Expt, DATA, plotres] = CombinePlot(DATA, chk);
    Expt.cp = ExptCP(Expt);
    if savefiles
        if j > length(cells)
            Expt.Header.cellnumber = 0;
            file = CombinedName(DATA,DATA.extype,1,'cell',mu);
            file = strrep(file,'.cell','.mu');
        else
            Expt.Header.cellnumber = cells(j);
            file = CombinedName(DATA,DATA.extype,1,'cell',cells(j));
        end
        c = '';
        nspk = sum([Expt.Trials.count]);
        save(file,'Expt');
...
function DATA = AddFitToData(DATA, plotres, fit)
    if isfield(plotres,'types')
        type = strmatch(plotres.types{1},{'Op' 'Pp'});
    else
...
elseif isfield(f,'Value')
    ...
    if next 
        if (DATA.subprobe == 0 || isempty(sit)) && id < length(probelist)
        id = id+1;
        set(pit,'value',id);
...
function DATA = CheckClusterLoaded(DATA, eid)
    if length(DATA.AllClusters) < eid || isempty(DATA.AllClusters{eid})
        ...
        for j = length(Clusters):-1:1
            ...
            if isfield(Clusters{j},'excludetrialids')
                DATA.AllClusters{eid}(j).excludetrialids = Clusters{j}.excludetrialids;
            else
                DATA.AllClusters{eid}(j).excludetrialids = [];
            end
            else
...
function Spikes = LoadGridSpikes(DATA, probe, eid)
%Load Spikes files made from .NS5 or .nev files
    Spikes = [];
    makedvdt = 1;
if ~isfield(DATA,'probes')
    ...
    id = find(DATA.grididx.expt == eid);
    if isempty(id)
        return;
    end
    [a,prefix] = fileparts(DATA.datadir);
    
    for j = 1:length(id)
        spkname = sprintf('%s/Spikes/%s.p%dt%d.mat',DATA.datadir, prefix, probe, eid);
        Spikes = GetProbeSpikes(DATA.AllData, spkname, [],1);
        
        if ~isfield(Spikes,'dVdt') && makedvdt
            Spikes.dVdt = diff(Spikes.values,[],2);
        end
    end
...
% available probes, so use DATA.probelist
 ...
 % multiple files (offline only)
        ...
        if DATA.state.online == 0
            if DATA.bysuffix
                eid = DATA.currentexpt;
                [d, a,b]  = fileparts(DATA.Expts{eid}.Header.Name);
                name = regexprep(a,'\.([0-9,a]*)$',['.p' num2str(DATA.probe) 't$1.mat']);
                filename = [d '/Spikes/' name];
                [DATA.AllData.Spikes]= GetProbeSpikes(DATA.AllData, filename, [],DATA.subprobe);
            elseif strncmp(DATA.filetype,'GridData',8)
                if ~isfield(DATA,'AllClusters')
                    DATA.AllClusters = {};
                end
