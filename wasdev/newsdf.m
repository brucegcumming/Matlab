function [sdf, nsac, nspikes, spikes] = newsdf(Trials, width, ...
						times, varargin)
% [sdf, nsac, nspikes, spikes] = trigsdf(Trials, width, times, ...)
%
...
for j = 1:length([Trials.Trial])
    Trials(j).psth = zeros(min(times):Trials(j).
  for t = [Trials(j).Trigger]
    spk = fix([Trials(j).Spikes'-t-times(1)]);
    idx = find(spk > 0 & spk <= nbins);
    if period
      idx = find(spk >= 0 & spk <= (times(end) - times(1)));
      spk = fix(mod(spk(idx)-t-times(1),period))+1;
      idx = 1:length(spk);
    end
...
elseif(strmatch('halfg',flag))
  ...
  fullsdf = [rates(1:end-3*width) .* 2 * 10000/(sqrt(2 * pi) * ...
						  width * nsac)];
elseif(strmatch('badbox',flag))
    for j = 1:length(times)-width
        rates(j) = sum(spikes(j:j+width));
    end
    rates(length(times)-width:length(times)+1) = 0;
        
    fullsdf = [rates(1:end) .* 2 * 10000/(sqrt(2 * pi) * ...
						  width * nsac)];
elseif(strmatch('box',flag))
  kl = ones(width,1);
  rates = conv(spikes,kl);
  fullsdf = [rates(1:end-width+1) .* 2 * 10000/(sqrt(2 * pi) * ...
						  width * nsac)];
